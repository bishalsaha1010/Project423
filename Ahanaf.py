...
def init_game():
    ...
    global shield_active, speed_boost_active, current_bg, garir_x
...
def draw_road():
    ...
    glPopMatrix()
def draw_sphere(radius=5, slices=16, stacks=16):
...
def draw_car(x, y, z, color):
    ...
    wheel_radius = 5
    wheel_positions = [
        (-garir_width/2 + 4, 2,  garir_length/3),
        ( garir_width/2 - 4, 2,  garir_length/3),
        (-garir_width/2 + 4, 2, -garir_length/3),
        ( garir_width/2 - 4, 2, -garir_length/3),
    ]
    ...
    for wx, wy, wz in wheel_positions:
        ...
        glRotatef(90, 0, 1, 0)     
        draw_sphere(radius=wheel_radius, width=4)
        glPopMatrix()
    glPopMatrix()
