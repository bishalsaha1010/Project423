...
def smart_autopilot():
    ...
    for i in obstacles:
        if car_z < i[2] < car_z + danger_zone:
            if i[0] < -road_width/6:
                lanes_blocked['left'] = True
            elif i[0] > road_width/6:
    ...
    if safe_lane:
        ...
        if abs(distance_to_target) > 5:
            car_x += car_speed * 1.2 if distance_to_target > 0 else -car_speed * 1.2
        else:
            car_x = target_x
...
def update():
    ...
    for pup in powerups[:]:
        ...
        if check_collision(car_x, car_z, car_width, car_length, pup_x, pup_z, 20, 20):
...
def draw_wings(x, y, z):
    ...
    car_color = [0,1, 0]  # Default green

    if shield_active and not fly_active:
        car_color = [0,0.5,1]  # Blue with shield

    if auto_pilot:
        car_color = [1, 1,0]  # Yellow in auto-pilot
